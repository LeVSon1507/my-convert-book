    ...
    function getTranslationHistoryId(fileHash, provider, model, chunkSize, scopePercent) {
      if (!fileHash || !provider || !model || !chunkSize) return '';
    ...
    function pushTranslationHistory(record) {
      try {
        ...
        if (historyId) {
          const existingIndex = list.findIndex(function(item) { return item?.historyId === historyId; });
          if (existingIndex >= 0) {
    ...
    function findLocalHistoryItem(historyId) {
      try {
        ...
        return list.find(function(item) { return item?.historyId === historyId; }) || null;
      } catch {
        return null;
      }
    }
    ...
    function updateLocalProgressHistoryEntry(status) {
      ...
      const chunkSize = Number.parseInt(document.getElementById('chunkSize').value, 10) || 6000;
      const scopePercent = getTranslationScopePercent();
      const checkpointKey = getTranslationCheckpointKey(currentFileHash, provider, modelName, chunkSize, scopePercent);
      const historyId = getTranslationHistoryId(currentFileHash, provider, modelName, chunkSize, scopePercent);
      ...
      if (localList.length > 0) {
        html += '<div class="history-section-label">💾 Chỉ trên thiết bị này</div>' +
          localList.map(function(item) {
            ...
            const progressText = total > 0 ? (done > 0 ? `${done}/${total} đoạn` : `${total} đoạn`) : '—';
            const statusText = item.status === 'in_progress' ? '⏸ Dở dang' : '✅ Hoàn tất';
            const canResume = item.status === 'in_progress' && item.checkpointKey && done > 0 && done < total;
            const canDownloadPartial = item.status === 'in_progress' && item.checkpointKey && done > 0;
            ...
            return '<div class="history-item">' +
              '<div class="history-item-info">' +
                ...
                '<div class="history-item-meta">' + progressText + ' · ' + statusText + '</div>' +
              '</div>' +
              '<div class="history-item-actions">' +
                (canDownloadPartial ? '<button class="btn btn-secondary btn-sm" onclick="downloadLocalHistoryPartial(\'' + hid + '\')">⬇️ Tải về</button>' : '') +
                (canResume ? '<button class="btn btn-secondary btn-sm" onclick="resumeLocalHistoryItem(\'' + hid + '\')">▶ Tiếp tục</button>' : '') +
              '</div>' +
            '</div>';
          }).join('');
      }
    ...
    async function resumeCloudHistoryItem(docId) {
      if (!currentFirebaseUser || typeof cloudLoadResumeCheckpoint !== 'function') return;
      try {
        ...
        if (promptTextEl) promptTextEl.textContent = `Checkpoint cloud: đã dịch ${done}/${total} đoạn. Tiếp tục?`;
    ...
    function downloadLocalHistoryPartial(historyId) {
      ...
      try {
        ...
        const parsed = JSON.parse(raw);
        const chunks = Array.isArray(parsed?.translatedChunks) ? parsed.translatedChunks.filter(Boolean) : [];
        if (!chunks.length) return showError('Checkpoint local chưa có nội dung dịch.');
        const baseName = (item.fileName || 'translated_partial').replace(/\.[^.]+$/, '');
        exportAs(chunks, `${baseName}_partial_${chunks.length}chunks_vietnamese`, 'txt');
      } catch (e) {
    ...
    function resumeLocalHistoryItem(historyId) {
      ...
      try {
        ...
        const parsed = JSON.parse(raw);
        const chunks = Array.isArray(parsed?.translatedChunks) ? parsed.translatedChunks : [];
        if (!chunks.length) return showError('Checkpoint local trống.');
        pendingResumeCheckpoint = { translatedChunks: chunks, totalChunks: Number(parsed?.totalChunks) || chunks.length };
        switchAppMode('translate');
        const done = chunks.filter(Boolean).length;
        const total = chunks.length;
        const resumePromptEl = document.getElementById('resumePrompt');
        const promptTextEl = document.getElementById('resumePromptText');
        if (promptTextEl) promptTextEl.textContent = `Checkpoint local: đã dịch ${done}/${total} đoạn. Tiếp tục?`;
        if (resumePromptEl) resumePromptEl.style.display = 'flex';
      } catch (e) {
    ...
    function loadFile(file) {
      ...
      reader.onload = async function(event) {
        ...
        if (!pendingResumeCheckpoint && currentFirebaseUser && typeof cloudFindResumeCandidate === 'function' && typeof cloudLoadResumeCheckpoint === 'function') {
          try {
            const cloudMeta = await cloudFindResumeCandidate(currentFileHash, provider, modelName, chunkSize, scopePercent);
            if (cloudMeta) {
              const cloudCheckpoint = await cloudLoadResumeCheckpoint(cloudMeta.id);
              const cloudList = Array.isArray(cloudCheckpoint?.translatedChunks) ? cloudCheckpoint.translatedChunks : [];
              const cloudDone = cloudList.filter(Boolean).length;
              if (cloudList.length === currentChunks.length && cloudDone > 0 && cloudDone < currentChunks.length) {
    ...
    function persistCurrentTranslationCheckpoint() {
      ...
      if (currentFirebaseUser && typeof cloudSaveTranslationProgress === 'function') {
