async function callWritingApi(systemPrompt, userPrompt) {
  ...
  const temperature = Number.parseFloat(
    document.getElementById("writingTemperature").value,
  );
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        if (code === "model_not_supported") {
          if (
            ...
            const fallbackModel =
              await globalThis.switchToSupportedHuggingFaceModel(modelName);
            if (fallbackModel) {
              modelName = fallbackModel;
              if (typeof addLog === "function") {
                addLog(
                  `♻️ Hugging Face: chuyển model fallback sang "${fallbackModel}" cho tính năng viết tiếp.`,
                  "warning",
                );
              }
              continue;
          ...
          throw new Error(
            "Model hiện tại không hỗ trợ chat/completions với provider Hugging Face hiện tại. Hãy dùng model từ dropdown tự nạp.",
          );
        }
      ...
      try {
        ...
        throw new Error(
          "JSON parse lỗi — response có thể bị cắt ngắn (timeout). Thử lại...",
        );
      }
      ...
      const isRetryable =
        apiCallError.message.includes("JSON parse") ||
      ...
      if (isRetryable && attempt < MAX_RETRIES) {
        ...
        console.warn(
          `Retry ${attempt + 1}/${MAX_RETRIES} sau ${retryDelay}ms:`,
...
async function startContinueWriting() {
  ...
  const chunkCount = Number.parseInt(
    document.getElementById("writingChunkCount").value,
    10,
  );
  ...
  document.getElementById("writingChunkIndicator").style.display =
    "inline-flex";
  const partialWritingBtn = document.getElementById(
    "downloadWritingPartialBtn",
  );
  ...
  document
    .getElementById("writingResultSection")
  ...
  const persistedAnalysis =
    !cachedStoryAnalysis && currentFileHash
      ? getCachedStoryAnalysis(currentFileHash)
      : null;
  if (persistedAnalysis) {
    cachedStoryAnalysis = persistedAnalysis;
    document.getElementById("analysisCacheHint").textContent =
      " ✅ Đã có bản phân tích cache (sẽ dùng lại)";
  ...
  if (isAnalysisEnabled) {
    if (cachedStoryAnalysis) {
      ...
      document.getElementById("writingOutput").textContent =
        "✅ Dùng lại bản phân tích đã có\n\n";
    } else {
      document.getElementById("writingOutput").textContent =
        "🔍 Đang phân tích truyện...\n";

      try {
        storyAnalysis = await analyzeFullStory(
          fileContent,
          function updateAnalysisStatus(statusMsg) {
  ...
  for (let chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++) {
    ...
    try {
      ...
      document.getElementById("writingPartialCount").textContent =
        continuedChunks.length;
      ...
      continuedChunks.push(
        `[LỖI ĐOẠN ${chunkIndex + 1}: ${writingError.message}]`,
      );
      renderWritingChunks();
  ...
  if (!isWritingStopped && continuedChunks.length === chunkCount) {
    ...
    document.getElementById("writingChunkIndicator").style.display =
      "inline-flex";
    const indicatorSpinner = document
      .getElementById("writingChunkIndicator")
