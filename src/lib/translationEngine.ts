...
function buildChatPayload(
  ...
  const { provider, model, temperature, systemPrompt, glossaryInstruction } = params;
  const messages = [
...
function extractUsageDelta(
  ...
  if (promptTokens <= 0 && completionTokens <= 0 && cost <= 0) return null;
...
async function translateChunkOnce(
  ...
  const usageDelta = extractUsageDelta(responseData);
...
async function resolveTranslationAttemptError(
  ...
  if (isContextLengthError(status, message) && attempt < maxRetries) {
    const splitText = await splitAndTranslateChunk(chunk, params, callApi, onUsage);
